  ...
  <main class="bg-gray-100 min-h-screen">
    <div class="mx-auto max-w-7xl px-4 py-6 sm:px-6 lg:px-8">
      <table class="min-w-full bg-white border rounded-md border-gray-200">
        ...
        <tbody>
          <tr v-for="allBooking in allBookings" :key="allBooking.id">
            <td class="py-2 px-4 border-b">
              <!-- Display the first ticket number or 'No ticket' if tickets are empty -->
              {{ allBooking.user?.name || "N/A" }}
            </td>
            <td class="py-2 px-4 border-b">
              {{ allBooking.tour?.name || "N/A" }}
            </td>
            <td class="py-2 px-4 border-b">
              {{ allBooking.tour?.destination?.name || "N/A" }}
            </td>
            <td class="py-2 px-4 border-b">
              <span
                v-if="allBooking.status === 'pending'"
                class="p-1 text-white rounded bg-yellow-500"
              >
                {{ allBooking.status || "N/A" }}
              </span>
            ...
            <td class="py-2 px-4 border-b">
              {{ new Date(allBooking.created_at).toLocaleDateString() }}
            </td>
            <td class="py-2 px-4 border-b">
              <button
                v-if="allBooking.status === 'confirmed'"
                @click="viewTicket(allBooking.id)"
              ...
              <button
                v-else
                @click="confirmBooking(allBooking.id)"
...
const allBookings = computed(() => store.state.allBookings);
